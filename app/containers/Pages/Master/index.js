...
import masterList from 'dan-api/ui/masterList';
...
class Master extends React.Component {
  ...
  handleClickOpen = (title, key) => {
    this.setState({ open: true, settingTitle: title, key });
  };
  ...
  render() {
    ...
    return (
      <div>
              ...
              {masterList.map((menu, index) => {
                ...
                return (
                  <Grid item key={index.toString()} sm={4} xs={12}>
                    <Button onClick={() => this.handleClickOpen(menu.name, menu.key)} color="secondary" className={classes.button}>
        ...
        <DetailView open={open} handleClose={this.handleClose} title={settingTitle} keyValue={key} />
      </div>
    );
  }
}
