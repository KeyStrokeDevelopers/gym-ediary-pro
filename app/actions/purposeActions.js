import {
  ...
  ERROR_PURPOSE_DATA, SHOW_DETAIL_PURPOSE, HIDE_DETAIL_PURPOSE, SUBMIT_PURPOSE_DATA, CLOSE_PURPOSE_FORM, LOADING_ACTION_PURPOSE, FETCH_ACCESS_DATA, CLOSE_PURPOSE_NOTIF, ACTIVE_PURPOSE_DATA, DELETE_PURPOSE_DATA, UPDATE_PURPOSE_DATA, COPY_PURPOSE_DATA
...
export const copyPurposeData = purposeData => ({
  type: COPY_PURPOSE_DATA,
  payload: purposeData
});
...
export const updatePurposeData = (data) => (dispatch) => {
  updatePurposeDataApi(data).then((response) => {
    dispatch(updatedPurposeData(response.data));
  }).catch((err) => {
    dispatch(errorPurposeData(err));
  });
};
